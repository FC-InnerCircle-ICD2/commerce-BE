...
@FeignClient(name = "elastic-service", url = "${search.find.feign-endpoint}", path = "/api/v1/elastic/product", configuration = FeignClientConfig.class)
public interface ElasticFeignClient {
    ...
    ResponseEntity<List<ElasticProduct>> getSimilarProducts(
        @PathVariable("productId") String productId,
        @RequestParam(value = "requestCount") Integer requestCount
    );
}
