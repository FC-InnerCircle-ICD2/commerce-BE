...
DEPLOY_FILES="member-service-1.0.jar Procfile .ebextensions .platform startup.sh"
...
# 2. build
./gradlew clean :member:build -x test

# 3. copy resources
cp -f member/build/libs/member-0.0.1.jar eb-deploy/${DEPLOY_APP}/
...
aws s3 cp eb-deploy/${DEPLOY_APP}/${DEPLOY_FILENAME} s3://innercircle-ecommerce-releases/member-service/${PROFILE}/${APP}/${DEPLOY_FILENAME} && \
aws elasticbeanstalk create-application-version --application-name ${APP} --version-label ${DEPLOY_VERSION_LABLE} --source-bundle S3Bucket=innercircle-ecommerce-releases,S3Key=member-service/${PROFILE}/${APP}/${DEPLOY_FILENAME} --no-paginate --no-cli-pager && \
